...
In @eq-stressall we use the following objects and symbols.
...
$D$ allows us carry out the summation of the weighted squared residuals in formula (@eq-stressall) over the upper (or lower) diagonal elements only. Thus we can also write
$$
...
@eq-stressall differs from Kruskal's original stress in at least three ways: in Kruskal's use of the square root, in our use of weights, and in our different approach to normalization.
